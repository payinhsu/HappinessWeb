...
pm2.connect(function(err) {
  ...
  bus.on('process:event', function(data) { 
     if (data.event === "exit") {
        ...
        slackbot.sendMsg("HappinessWeb:Happiness Web Crash: event: " + data.event + ", pm_exec_path: " + data.process.pm_exec_path , ":broken_heart:", function(success, slackRes){
          if(success){
  ...
  bus.on('log:err', function(data) { 
    if(data.data.toLowerCase().indexOf('warn') != 0){
      ...
      slackbot.sendMsg("HappinessWeb:log:err: " + JSON.stringify(data) , ":angry:", function(success, slackRes){
        if(success){
          console.log("Slack Response: ", JSON.stringify(slackRes));
        }else{
          console.log("Slack error: ", JSON.stringify(slackRes));
